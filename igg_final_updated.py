...
class MAIN:
   ...
   def __init__(self):
       super(MAIN).__init__()
       self.head = {'user-agent': 'Dalvik/2.1.0 (Linux; U; Android 13; moto g82 5G Build/T1SU33.1-124-6-8) 38.0.0.13.95 Android (26/8.0.0; 320dpi; 720x1184; Sony; F5321; F5321; qcom; ru_RU; 99640905)',}
...
[white]Masukan Nama File Untuk Melihat Hasil, Gunakan koma untuk pemisahan nama file',True)
   ...
   def Exekusi(self, uid, cokie, next, mode,unli=None):
       global xx
       headers = {'Host': 'www.instagram.com','x-requested-with': 'XMLHttpRequest','x-csrftoken': re.search('csrftoken=(.*?);',str(cokie['cookie'])).group(1),'x-ig-app-id': '1217981644879628','user-agent': 'Dalvik/2.1.0 (Linux; U; Android 13; motorola edge 30 pro Build/T1SHS33.35-23-20-14)','accept-language': 'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7',}
   ...
   def Graphql(self, typess, userid, cokie,after):
       ...
       try:
           self.ptk = {"user-agent": "Dalvik/2.1.0 (Linux; U; Android 13; SHG11 Build/SB069)","accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7","cookie": cokie}
...
- Mainkan Mode Pesawat Jika Proses Cepat 400 ID Slow 200 -')
   ...
   def friends_user(self, cookies):
       try:
            InfoHeaders = {'x-ig-app-locale': 'in_ID','x-ig-device-locale': 'in_ID','x-ig-mapped-locale': 'id_ID','x-bloks-version-id': '8ca96ca267e30c02cf90888d91eeff09627f0e3fd2bd9df472278c9a6c022cbb','x-bloks-is-layout-rtl': 'false','x-ig-capabilities': '3brTv10=','x-ig-app-id': '567067343352427','priority': 'u=3','user-agent': 'Dalvik/2.1.0 (Linux; U; Android 13; octopus Build/R123-15786.58.0) (25/7.1.2; 240dpi; 720x1280; Google/google; google Pixel 2; x86; android_x86; in_ID; 458229257)','accept-language': 'id-ID, en-US','x-fb-http-engine': 'Liger','x-fb-client-ip': 'True','x-fb-server-cluster': 'True'}
            edit = {'edit': 'true'}
   ...
   def HeadersApiLogin(self):
       return {
          ...
          'priority': 'u=3',
          'user-agent': 'Dalvik/2.1.0 (Linux; U; Android 13; octopus Build/R123-15786.58.0) (25/7.1.2; 240dpi; 720x1280; Google/google; google Pixel 2; x86; android_x86; in_ID; 458229257)',
 ...
 {P}data profile : {K2}{followers}/{following}/{postingan}                             ''')
   ...
   def headers_graph(self, xxx):
       headers = {
           ...
           'user-agent': 'Dalvik/2.1.0 (Linux; U; Android 13; WP33 Pro Build/TP1A.220624.014)',
